...
- Add `u8aWrapBytes` & `u8aUnwrapBytes` (originally from `@axia-js/extension-dapp`)
...
- Sync with upstream Axlib ss58 registry
...
- Ensure `@axia-js/util-crypto` doesn't re-export `packageInfo` from `@axia-js/networks`
- Minor TS style cleanups in `@axia-js/x-bundle`
...
- **Breaking change** The `@axia-js/x-rxjs` package has been removed. In `rxjs` 7.2.0 the compatibility issues at the base layer has been solved.
- **Breaking change** The named exports in `@axia-js/networks` has been changed
...
- Adjust `@axia-js/networks` to fully-descriptive exports
- Remove `@axia-js/x-rxjs`, direct import from `rxjs >= 7.2.0` required
...
- Cleanup unneeded dependencies for `@axia-js/x-global`
- Sync with upstream Axlib ss58 registry
...
- Aligned ss58 registry with Axlib
...
- Adapt `@axia-js/x-global` to return `globalThis` as available
...
- Check `@axia-js/hw-ledger` against `@axia-js/networks` for discrepancies (test-only)
- Align ss58 registry with upstream Axlib
...
- Adjust Polymath genesis in `@axia-js/networks` (Thanks to https://github.com/adamdossa)
- Adjust `@axia-js/util` logger for +/-/* compat (Thanks to https://github.com/Lezek123)
- Fix typo in `@axia-js/util` detect messaging (Thanks to https://github.com/amphineko)

Changes:

- Remove `@axia-js/networks` code duplication, manages data via script from upstream
- Align ss58 registry with upstream Axlib
...
- Fix for `@axia-js/x-rxjs` in Node CJS environments (no Node-specific import)
...
- Fix `@axia-js/x-rxjs` ESM exports for Node environments (now different due to the ESM conversion)
...
- Fix package detection for `@axia-js/networks` (no dependencies)
...
- Cleanup formatting on nested conditionals in the `@axia-js/x-` packages
...
- Add additional ss58 test vectors (against chains with >63 prefixes in the Axlib registry)
...
- `@axia-js/networks` now filters the new upstream-introduced `null` networks
...
- Fix installation instructions in `@axia-js/x-global` (Thanks to https://github.com/raoulmillais)
...
- Align `@axia-js/networks` with latest ss58 registry
...
- Add `slip44` indices for Ledger into `@axia-js/networks`
- Remove `module` entry in `package.json` (only rely on exports map)
- Remove direct references to `global` object, use `@axia-js/x-global`
...
- Add `schnorrkelVrf{Sign, Verify}` to `@axia-js/util-crypto`
- Export `vrf{Sign, Verify}` on keyring interface
- Move `@axia-js/x-rxjs` packages from the api repo
- Align `@axia-js/networks` with Axlib master
- Explicit edcsa tests for derivation
- `@axia-js/wasm-crypto` 3.2.1


## 5.3.1 Jan 17, 2021

**Important** For users of the `@axia-js/ledger` package, it is now included in this repo (since it is a general utility) and renamed to `@axia-js/hw-ledger`
...
- Move `@axia-js/ledger` packages into common
- Update `@axia-js/networks` to align with latest Axlib ss58-registry
...
- Align with Axlib master network list
...
- Add explicit link to base Axlib ss58 formats (as managed in `@axia-js/util`)
...
- Update `@axia-js/wasm-crypto` library (under-the-hood cleanups)
- Remove explicit crypto polyfill injection (aligning with updated `@axia-js/wasm-crypto`)
- Add cross-environment `@axia-js/x-randomvalues` (for use in `@axia-js/wasm-crypto`)
...
- Add Totem (ss58 prefix 14) into `@axia-js/networks`
- Move `@axia-js/x-{fetch,textdecoder,textencoder,ws}` into `common` repo
...
- Add evmToAddress and addressToEvm `@axia-js/util-crypto` (Thanks to https://github.com/jnaviask)
- Remove invalid link to docs (thanks to https://github.com/roccomuso)

Changes:

- Rename `mnemonicToBip39` to `mnemonicToLegacySeed` (used in non-Axlib compatible fashion)
- Update pair signature verification to handle MultiSignature results
- Move documentation to docs repo
- Added `@axia-js/networks` for a list of known networks and their identifying information
...
- Use cross-environment TextDecoder & TextEncoder from the axia-js polyfills
- Add `isBase{32, 58, 64}` to `@axia-js/util-crypto`
...
- Update to `@axia-js/wasm-crypto` 1.3.1
...
- Rename `createKeySub` to `createKeyDerived` based on changed in Axlib (sub keys now named derived)
...
- Adjust keyring to no override 0-prefix (AXIA live) settings with keychain default
...
- **Important** Removed `mnemonicToSeed`, it is only for bip39, use `mnemonicToMiniSecret` for the Axlib variant
...
- Cleanup `isDevelopment` regex to cater for chains such as `AlphaNet Development`
...
- Allow for optional wasm-crypto, specifically via `@axia-js/util-crypto/noWasm`
- Misc. CI infrastructure cleanups


## 2.6.1 Feb 29, 2020

- **Breaking change** Following on the 2.0 release, `@axia-js/wasm` was updated to 1.1+. sr25510 now only verifies 0.8+ signatures, closing the loop on dropping Axlib 1.x-era support
...
- Adjust `formatBalance(<balance>, <options>)` to take expanded `withUnit: string | boolean` option where string is an actual unit, e.g. `AXCT`
- The `decimals` option to `formatBalance` should now be passed as part of the `options` (previous last-param deprecated)
- The `.setSS58Format` on keyring will now just operate on pairs created on the keyring, not globally. The global `setSS58Format` in `@axia-js/util-crypto` will be deprecated.
...
- Update to `@axia-js/wasm-crypto` 1.0
...
- Update w3f/schnorrkel to 0.8.5 (Full Axlib 2.x support, no 1.x support)
- Remove Alice session (ed2551) account from testing keyring (not applicable to Axlib 2.x)
...
- `pair.sign(message)` now takes an optional second `options?: SignOptions` parameter, where `SignOptions` is currently defined as `{ withType: boolean }`. Here `withType` instructs the signing to prepend the type of signature (ed2551, sr25519 or ecdsa). This is used for the new AXIA/Axlib MultiSignature format.
...
- Added `stringToHex` and `hexToString` conversion utilities to `@axia-js/util`
...
- Add `checkAddress(address, prefix)` to `@axia-js/util-crypto` that validates an ss-58 address
- Add support for the AXIALunar network (as a valid checked prefix)
- Add an `asm.js` fallback for the React Native environment (via `@axia-js/wasm-crypto`)
...
- `Move decodeAddress`, `encodeAddress` & `setAddressPrefix` functions into `@axia-js/util-crypto` from `@axia-js/keyring`. External interfaces should not be affected at this point since it is also (still) exported and exposed on keyring
...
- Update `@axia-js/wasm` to include a maintenance bump for the `w3f/schnorrkel` libraries
...
- Pull in `formatBalance`, `formatDecimal`, `formatElapsed`, `formatNumber`, `calcSi`, `findSi` & `isTestChain` utility functions from the originals in `@axia-js/ui-util`
...
- Align with current axlib master for sr25519 keys and hard/soft derivation
