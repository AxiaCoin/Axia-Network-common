...
describe('checkAddress', (): void => {
  it('returns [true, null] for AXIALunar', (): void => {
    expect(
      checkAddress('FJaco77EJ99VtBmVFibuBJR3x5Qq9KQrgQJvWjqScCcCCae', 2)
    ).toEqual([true, null]);
  });

  it('returns [true, null] for Axlib', (): void => {
    expect(
  ...
  it('fails with invalid prefix when checking Axlib against AXIALunar prefix', (): void => {
    expect(
